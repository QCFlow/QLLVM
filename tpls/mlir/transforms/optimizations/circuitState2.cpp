...
namespace qllvm {
void circuitState2::getDependentDialects(DialectRegistry &registry) const {
  registry.insert<LLVM::LLVMDialect>();
}


void circuitState2::runOnOperation() {
  if (flag == true)
    *c+=1;
  ...
  // std::cout<<"top node number: "<<top_op.size()<<"   is: "<<std::endl;
  for (auto &op : top_op) {
  ...
  int cx_num = 0;
  std::cout <<"======================================"<<std::endl;
  std::cout << "after optimization, the total number of quantum gates: " << gateCount <<std::endl;
