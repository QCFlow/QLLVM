...
struct ConsolidateBlocks
    : public PassWrapper<ConsolidateBlocks, OperationPass<ModuleOp>> {
  void getDependentDialects(DialectRegistry &registry) const override;
  void runOnOperation() final;
  ConsolidateBlocks() {};
  ConsolidateBlocks(std::unordered_set<std::string> basicgate){
    basic_gate = basicgate;
  }
  ConsolidateBlocks(std::map<std::string, bool> bool_args,int &opt_count, int &opt_depth, int &cir_depth, int &zero_count, int &enable, int &pass_count) {
  ...
  std::string passname = "ConsolidateBlocks";
  };
}