...
bool emitInstruction(CallInst* CI, const QubitMap& qubitMap,
  ...
  if (name == "__quantum__qis__phasedx") {
    double theta_1 = getConstantDouble(CI->getArgOperand(0));
    double theta_2 = getConstantDouble(CI->getArgOperand(1));
    int q = getQubitIndex(CI->getArgOperand(2), qubitMap);
    if (q >= 0) {
      out << "U1q (" << std::setprecision(17) << theta_1 << "*pi, " << std::setprecision(17) << theta_2 << "*pi) q[" << q << "];\n";
    }
    return q >= 0;
  }
...
std::string QirToIonQasmTranslator::translate(llvm::Module* module,
...
bool QirToIonQasmTranslator::translateToFile(llvm::Module* module,
...
std::string QirToIonQasm(llvm::Module* module, const std::string& kernelName) {
  QirToIonQasmTranslator translator;
  return translator.translate(module, kernelName);
}

bool QirToIonQasmFile(llvm::Module* module, const std::string& outPath,
                   const std::string& kernelName) {
  QirToIonQasmTranslator translator;
