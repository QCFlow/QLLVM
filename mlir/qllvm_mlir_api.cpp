...
#include "qllvm_config.hpp"
#include "quantum_to_llvm.hpp"
// #include "tools/ast_printer.hpp"
#include "tools/qllvm-mlir-helper.hpp"
...
const std::string mlir_compile(const std::string &src,
  ...
  auto mlir_gen_result =
      qllvm::util::mlir_gen(src, kernel_name, add_entry_point, extra_args);
  ...
  if (opt_level > 0) {
    qllvm::configureOptimizationPasses(pm);
  }
  pm.addPass(std::make_unique<qllvm::ModifierRegionRewritePass>());
  pm.addPass(std::make_unique<qllvm::QuantumToLLVMLoweringPass>(
...
int execute(const std::string &src, const std::string &kernel_name,
  ...
  std::cerr << "execute()/JIT is not available in OpenQASM-only build.\n";
  return 1;
}
...
// JIT execute implementations removed (OpenQASM-only)
#if 0
int __deleted(const std::string &src, const std::string &kernel_name,
            int opt_level, std::map<std::string, std::string> extra_args) {
  mlir::registerAsmPrinterCLOptions();
  mlir::registerMLIRContextCLOptions();

  auto mlir_gen_result = qllvm::util::mlir_gen(src, kernel_name, true, extra_args);
  mlir::OwningModuleRef &module = *(mlir_gen_result.module_ref);
  mlir::MLIRContext &context = *(mlir_gen_result.mlir_context);
  std::vector<std::string> &unique_function_names =
      mlir_gen_result.unique_function_names;
  ...
  jit.jit_compile(
      ...
      std::vector<std::string>{
          std::string(QLLVM_INSTALL_DIR) + std::string("/lib/libqir-qrt") +
              std::string(QLLVM_LIB_SUFFIX),
...
int execute_removed2(const std::string &src, const std::string &kernel_name,
            std::vector<std::unique_ptr<llvm::Module>> &extra, int opt_level) {
  mlir::registerAsmPrinterCLOptions();
  mlir::registerMLIRContextCLOptions();

  auto mlir_gen_result = qllvm::util::mlir_gen(src, kernel_name, true);
  mlir::OwningModuleRef &module = *(mlir_gen_result.module_ref);
  mlir::MLIRContext &context = *(mlir_gen_result.mlir_context);
  std::vector<std::string> &unique_function_names =
      mlir_gen_result.unique_function_names;
  ...
  jit.jit_compile(std::move(llvmModule),
                  std::vector<std::string>{std::string(QLLVM_INSTALL_DIR) +
