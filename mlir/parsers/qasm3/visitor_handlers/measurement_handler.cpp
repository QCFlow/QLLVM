  ...
  if (context->EQUALS() || context->ARROW()) {
    ...
    if (value.getType() == qubit_type) {
      ...
      if (auto index_list =
        ...
        auto idx_txt = index_list->expression(0)->getText();
        if (auto opt =
                symbol_table.try_evaluate_constant_integer_expression(idx_txt))
          validate_classical_reg_index(bit_value, bit_variable_name, *opt,
                                       location);
