...
/// Declared length of a quantum register (qreg / qubit[]), if known at compile time.
static std::optional<uint64_t> get_declared_qreg_size(
    mlir::Value v, const std::string &qreg_name,
    ScopedSymbolTable &symbol_table, int depth = 0) {
  if (depth > 32)
    return std::nullopt;
  if (auto *op = v.getDefiningOp()) {
    if (auto qa = mlir::dyn_cast<mlir::quantum::QallocOp>(op))
      return qa.size().getLimitedValue();
    if (auto alias = mlir::dyn_cast<mlir::quantum::QaliasArrayAllocOp>(op))
      return alias.size().getLimitedValue();
    if (auto ex = mlir::dyn_cast<mlir::quantum::ExtractQubitOp>(op))
      return get_declared_qreg_size(ex.qreg(), qreg_name, symbol_table,
                                    depth + 1);
  }
  const auto attrs = symbol_table.get_variable_attributes(qreg_name);
  if (!attrs.empty()) {
    try {
      uint64_t n = static_cast<uint64_t>(std::stoll(attrs[0]));
      return n;
    } catch (...) {
    }
  }
...
void printErrorMessage(const std::string msg,
  ...
  std::cout << "\n[OPENQASM MLIRGen] Error at " << line << ":" << col << "\n"
...
std::optional<int64_t> try_memref_index_constant(mlir::Value idxVal) {
  mlir::Value v = idxVal;
  for (int depth = 0; depth < 16; ++depth) {
    if (auto cop = v.getDefiningOp<mlir::ConstantOp>()) {
      auto attr = cop.getValue();
      if (auto intAttr = attr.dyn_cast<mlir::IntegerAttr>())
        return intAttr.getInt();
      return std::nullopt;
    }
    if (auto ici = v.getDefiningOp<mlir::IndexCastOp>()) {
      v = ici.getOperand();
      continue;
    }
    if (auto zext = v.getDefiningOp<mlir::ZeroExtendIOp>()) {
      v = zext.getOperand();
      continue;
    }
    if (auto trunc = v.getDefiningOp<mlir::TruncateIOp>()) {
      v = trunc.getOperand();
      continue;
    }
    break;
...
void validate_classical_reg_index(mlir::Value memref, const std::string &reg_name,
                                  int64_t idx, mlir::Location location,
                                  bool do_exit) {
  if (idx < 0) {
    std::string msg = "classical register index must be non-negative: '" +
                      reg_name + "'[" + std::to_string(idx) + "].";
    {
      std::string locStr;
    ...
    printErrorMessage(msg, do_exit);
    return;
  }
  auto mt = memref.getType().dyn_cast<mlir::MemRefType>();
  if (!mt)
    return;
  ...
  if (mt.getRank() == 0) {
    if (uidx != 0) {
      std::string msg =
          "classical register index out of range: '" + reg_name + "'[" +
          std::to_string(idx) +
          "] — scalar bit/creg has only index 0 (memref rank 0).";
      {
        std::string locStr;
        llvm::raw_string_ostream rso(locStr);
        location.print(rso);
  ...
  if (mt.getRank() != 1)
    return;
  int64_t dim = mt.getDimSize(0);
  if (mlir::ShapedType::isDynamic(dim))
    return;
  uint64_t n = static_cast<uint64_t>(dim);
  if (uidx >= n) {
    std::string msg =
        "classical register index out of range: '" + reg_name + "'[" +
        std::to_string(idx) + "] but '" + reg_name + "' has size " +
        std::to_string(n) + " (valid indices: 0.." + std::to_string(n - 1) + ").";
    {
      std::string locStr;
      llvm::raw_string_ostream rso(locStr);
      location.print(rso);
...
mlir::Value get_or_extract_qubit(const std::string &qreg_name,
  ...
  if (symbol_table.has_symbol(key)) {
    ...
    if (auto regSize =
            get_declared_qreg_size(qubits, qreg_name, symbol_table)) {
      if (idx >= *regSize) {
        std::string msg =
            "quantum register index out of range: '" + qreg_name + "'[" +
            std::to_string(idx) + "] but '" + qreg_name + "' has size " +
            std::to_string(*regSize) +
            " (valid indices: 0.." + std::to_string(*regSize - 1) + ").";
        {
          std::string locStr;
          llvm::raw_string_ostream rso(locStr);
          location.print(rso);
