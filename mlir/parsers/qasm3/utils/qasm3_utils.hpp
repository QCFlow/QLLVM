...
/// If \p idxVal is a constant (arith constant, index cast chain, zext), return it.
std::optional<int64_t> try_memref_index_constant(mlir::Value idxVal);

/// Abort if \p idx is outside the static shape of \p memref (bit/creg array).
/// Rank-0 memref is treated as length 1. Dynamic dimensions skip the check.
void validate_classical_reg_index(mlir::Value memref,
                                  const std::string &reg_name, int64_t idx,
                                  mlir::Location location, bool do_exit = true);
