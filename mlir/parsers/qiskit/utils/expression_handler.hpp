...
namespace qllvm {
class qiskit_expression_generator : public qiskit::qiskitBaseVisitor {
 protected:
 ...
 public:
  ...
  qiskit_expression_generator(mlir::OpBuilder& b, ScopedSymbolTable& table,
                             std::string& fname);
  qiskit_expression_generator(mlir::OpBuilder& b, ScopedSymbolTable& table,
                             std::string& fname, mlir::Type t);

  antlrcpp::Any visitTerminal(antlr4::tree::TerminalNode* node) override;
  antlrcpp::Any visitExpression(qiskitParser::ExpressionContext* ctx) override;

  antlrcpp::Any visitExpressionTerminator(
      qiskitParser::ExpressionTerminatorContext* ctx) override;
};
}  // namespace qllvm