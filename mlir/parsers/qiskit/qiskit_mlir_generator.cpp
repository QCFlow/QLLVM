...
#include "antlr/generated/qiskitLexer.h"
#include "antlr/generated/qiskitParser.h"
namespace qllvm {

void QiskitMLIRGenerator::initialize_mlirgen(
  ...
  visitor = std::make_shared<qiskit_visitor>(builder, m_module, file_name);
...
void QiskitMLIRGenerator::mlirgen(const std::string &src) {
  ...
  qiskitLexer lexer(&input);
  CommonTokenStream tokens(&lexer);
  qiskitParser parser(&tokens);

  lexer.removeErrorListeners();
  parser.removeErrorListeners();
  // std::cout << "to get program" << std::endl;
  tree::ParseTree *tree = parser.program();
  // std::cout << "start to visitChildren" << std::endl;
  visitor->visitChildren(tree);
  // std::cout << "finish " << std::endl;
  return;
}

void QiskitMLIRGenerator::finalize_mlirgen() {
