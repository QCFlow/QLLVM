...
struct SingleQubitGateMergingPass
  ...
  ~SingleQubitGateMergingPass() {}
  SingleQubitGateMergingPass(std::unordered_set<std::string> basicgate){
    basic_gate = basicgate;
  }
  SingleQubitGateMergingPass(std::map<std::string, bool> bool_args,int &opt_count, int &opt_depth, int &cir_depth, int &zero_count, int &enable, int &pass_count) {
...
private:
  ...
  std::string passname = "SingleQubitGateMergingPass";
};
} // namespace qllvm