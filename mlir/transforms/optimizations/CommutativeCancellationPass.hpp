...
struct CommutativeCancellationPass
    : public PassWrapper<CommutativeCancellationPass, OperationPass<ModuleOp>> {
  void getDependentDialects(DialectRegistry &registry) const override;
  void runOnOperation() final;
  CommutativeCancellationPass() {}
  CommutativeCancellationPass(std::map<std::string, bool> bool_args,int &opt_count, int &opt_depth, int &cir_depth, int &zero_count, int &enable, int &pass_count) {
...
std::string passname = "CommutativeCancellationPass";
};
}
