...
struct RemoveIdentityEquivalent
    : public PassWrapper<RemoveIdentityEquivalent, OperationPass<ModuleOp>> {
  void getDependentDialects(DialectRegistry &registry) const override;
  void runOnOperation() final;
  RemoveIdentityEquivalent() {}
  RemoveIdentityEquivalent(std::map<std::string, bool> bool_args,int &opt_count, int &opt_depth, int &cir_depth, int &zero_count, int &enable, int &pass_count) {
...
private:
  ...
  const double MINIMUM_TOL = 1e-12;
  bool syn = false;
  bool f = false;
  int *p = nullptr;
  int *q = nullptr;
  ...
  std::string passname = "RemoveIdentityEquivalent";
};
} // namespace qllvm