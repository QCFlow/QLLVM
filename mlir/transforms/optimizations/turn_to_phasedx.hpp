...
struct turn_to_phasedx
    : public PassWrapper<turn_to_phasedx, OperationPass<ModuleOp>> {
  void getDependentDialects(DialectRegistry &registry) const override;
  void runOnOperation() final;
  turn_to_phasedx() {}
  ~turn_to_phasedx() {}
  turn_to_phasedx(std::map<std::string, bool> bool_args,int &opt_count, int &opt_depth, int &cir_depth, int &zero_count, int &enable, int &pass_count) {
...
private:
  ...
  std::string passname = "turn_to_phasedx";
};
} // namespace qllvm